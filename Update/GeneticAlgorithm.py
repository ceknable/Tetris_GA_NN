...
def simulate_brain(DNA):
    ...
    fitness = total_score*3 - holes - avg_height*10  # Higher fitness is better
    ...
    for i in range(1, len(chromosome)):
        if random.random() < mutation_rate: # check if there should be a mutation
            ...
            # mutation for number of neurons in layer
                ...
                chromosome[i] += random.randint(-3,3) #random.randint(low_N, up_N)
...
def genetic_algorithm(population_size, generations, mutation_rate_0, mutation_rate_14, low_bound, up_bound, low_N, up_N):
    ...
    for generation in range(generations):
        ...
        for _ in range((population_size-3) // 4):  # Half-size as 2 children are generated per pair
            ...
            child3 = mutate(child3, mutation_rate_0, mutation_rate_14, low_bound, up_bound, low_N, up_N)
            child4 = mutate(child4, mutation_rate_0, mutation_rate_14, low_bound, up_bound, low_N, up_N)
        ...
        genchamps.append([best_individual, best_fitness, best_seed, avg_fitness])
        if (generation+1)%1:
            None
        else:
            print(f"Generation {generation + 1}: Best Fitness = {best_fitness} ; Avg = {avg_fitness}")
