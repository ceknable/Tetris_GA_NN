...
def simulate_brain(DNA):
    ...
    fitness = total_score*3 - 12*holes - avg_height*20  # Higher fitness is better
...
def genetic_algorithm(population_size, generations, mutation_rate_0, mutation_rate_14, low_bound, up_bound, low_N, up_N):
    ...
    for generation in range(generations):
        ...
        if generation > 11:
            ...
            avgf = np.round(np.average(afitnesses),2)
            ...
            abs_thresh = (genchamps[generation-1][1]-40)
            ...
            if avgf == np.round(genchamps[generation-1][1],2):
        ...
        if (generation+1)%1:
            ...
            print(f"Generation {generation + 1}: Best Fitness = {np.round(best_fitness,2)} ; Avg = {np.round(avg_fitness,2)}")
